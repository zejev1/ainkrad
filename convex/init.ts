...
const init = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // =====================================================
    //
    // Создаём стартовое поселение.
    //
    // seedStarterSettlement идемпотентный:
    // если здания уже существуют,
    // второй комплект он не создаст.
    // =====================================================
    ...
    // Cardinal пока намеренно НЕ запускаем.
    //
    // Сначала строим полноценный мир:
    // жильё -> работа -> ресурсы ->
    // потребности -> экономика ->
    // строительство.
    //
    // После этого Cardinal Observer
    // сможет наблюдать уже настоящее
    // возникающее общество.
  },
});
